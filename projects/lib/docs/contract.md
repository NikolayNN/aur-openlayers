...
- Если hit-test вернул несколько фич одного слоя (например, несколько объектов в одной точке),
  состояние `select.state` применяется ко всем элементам `items` этого слоя.
- `select.pickTargets` позволяет отфильтровать/выбрать нужные элементы из `candidates`; в `onSelect`
  попадут только выбранные элементы.
- Если `select.pickTargets` вернул `null/undefined` или пустой массив, `select` игнорируется
  для текущего клика (без `onSelect`, `onClear` и без изменения `select.state`).
